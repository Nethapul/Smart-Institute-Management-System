          ...
          <table class="table bordered-table mb-0 data-table" id="dataTable">
            ...
            <tbody>
              <tr v-for="(teacher, index) in teachers" :key="index">
                <td>
                  <div class="form-check style-check d-flex align-items-center">
                    <input class="form-check-input" type="checkbox" v-model="selectedItems" :value="teacher.id">
                    <label class="form-check-label">{{ index + 1 < 10 ? '0' + (index + 1) : index + 1 }}</label>
                  </div>
                </td>
                <td><span class="text-primary-600">{{ teacher.admissionNo }}</span></td>
                <td>
                  <div class="d-flex align-items-center flex-grow-1">
                    <img :src="teacher.image" alt="Teacher Image" class="flex-shrink-0 me-12 radius-8" width="40" height="40">
                    <div class="">
                      <h6 class="text-md mb-0 fw-medium flex-grow-1">{{ teacher.name }}</h6>
                    </div>
                  </div>
                </td>
                  ...
                  <div class="d-flex align-items-center flex-wrap gap-28">
                    <div class="form-check checked-primary d-flex align-items-center gap-2" v-for="type in attendanceTypes" :key="type.value">
                      <input 
                        ...
                        :id="'attendance_' + type.value + '_' + teacher.id"
                        :value="type.value"
                        v-model="teacher.attendance"
                      >
                      <label class="form-check-label" :for="'attendance_' + type.value + '_' + teacher.id">{{ type.label }}</label>
                    </div>
                  </div>
                </td>
                <td>
                  <input type="text" class="form-control" placeholder="Write note..." v-model="teacher.note">
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  setup() {
    ...
    const teachers = ref([
      { id: 1, admissionNo: 'AD52365', name: 'Marvin McKinney', subject: 'Mathematics', image: img1, attendance: '', note: '' },
      { id: 2, admissionNo: 'AD52366', name: 'Cody Fisher', subject: 'Physics', image: img2, attendance: '', note: '' },
      { id: 3, admissionNo: 'AD52367', name: 'Jenny Wilson', subject: 'Biology', image: img3, attendance: '', note: '' },
      { id: 4, admissionNo: 'AD52368', name: 'Guy Hawkins', subject: 'English', image: img4, attendance: '', note: '' },
      { id: 5, admissionNo: 'AD52369', name: 'Esther Howard', subject: 'Math', image: img5, attendance: '', note: '' },
      { id: 6, admissionNo: 'AD52370', name: 'Jane Cooper', subject: 'Chemistry', image: img6, attendance: '', note: '' },
      { id: 7, admissionNo: 'AD52371', name: 'Robert Fox', subject: 'Biology', image: img7 || img1, attendance: '', note: '' },
      { id: 8, admissionNo: 'AD52372', name: 'Theresa Webb', subject: 'Mathematics', image: img8 || img2, attendance: '', note: '' },
      { id: 9, admissionNo: 'AD52373', name: 'Courtney Henry', subject: 'Physics', image: img3, attendance: '', note: '' }, // Reusing img3
      { id: 10, admissionNo: 'AD52374', name: 'Wade Warren', subject: 'Biology', image: img6, attendance: '', note: '' },  // Reusing img6
    ]);

    const toggleAll = () => {
      if (selectAll.value) {
        selectedItems.value = teachers.value.map(s => s.id);
      } else {
        selectedItems.value = [];
      }
    };
